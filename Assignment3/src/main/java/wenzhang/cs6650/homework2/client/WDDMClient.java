...
public class WDDMClient {
	...
	public static void main(String[] args) throws InterruptedException {
		WDDMClient wddmCl = new WDDMClient();
	    ...
	    Response response = initialWebtarget.request().post(Entity.json(""));
...
//	    }
	...
	public boolean clientThroughputChart(Options options) {
		...
		for (Map.Entry<Integer, Integer> entry : WDDMClient.synThroughputData.entrySet()) { // loop through the throughput map to add points to the chart
	...
	public void LatencyMeasurement(Integer totalrequest, long duration) {
	    ...
	    try {
	        ...
	        while ((line = br.readLine()) != null) {
	            ...
	            for (int i = 0; i < latencies.length; i++) {
	            	WDDMClient.latencylist.add(Long.valueOf(latencies[i])); // translate each data to long value of latency
	            }
	        }
		...
		WDDMClient.latencylist.sort((Long x,Long y)->(x.compareTo(y))); // sort the latencies to calculate the mean, median, and percentile information
		long sum = 0;
		int size = WDDMClient.latencylist.size();
		for(int i = 0; i < size; i++) {
			sum = sum + WDDMClient.latencylist.get(i);
		}
		System.out.println("Mean latencies for all requests: " + sum / size + " MilliSeconds*");
		int medianindex = (size - 1) / 2;
		System.out.println("Median latencies for all requests: " + WDDMClient.latencylist.get(medianindex) + " MilliSeconds*");
		int percentindex95 = (int) (size * 0.95);
		int percentindex99 = (int) (size * 0.99);
		System.out.println("95th percentile latency for all requests: " + WDDMClient.latencylist.get(percentindex95) + " MilliSeconds*");
		System.out.println("99th percentile latency for all requests: " + WDDMClient.latencylist.get(percentindex99) + " MilliSeconds*");
	}
}
